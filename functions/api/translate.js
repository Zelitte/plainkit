// Prekladová funkcia: overuje Turnstile, kontroluje limity, prepošle na Gemini.
...
export async function onRequestPost(context) {
  ...
  if (!turnstileToken || typeof turnstileToken !== 'string') return jsonError(401, 'MISSING_TURNSTILE');

  const tsOk = await verifyTurnstile(turnstileToken, ip, env);
  if (!tsOk) return jsonError(401, 'TURNSTILE_FAILED');
